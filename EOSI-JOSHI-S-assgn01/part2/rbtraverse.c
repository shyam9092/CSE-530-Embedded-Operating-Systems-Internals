...
void kp_rbtree_sniff(struct rb_root *root, struct rb_user_buff *buff, int size)
{
	for(int i=0; i<=size; i++)
	{
		printk("Entered kp_rbtree_sniff \n");
	}
}
EXPORT_SYMBOL_GPL(kp_rbtree_sniff);
...
EXPORT_SYMBOL_GPL(rb_driver_write);
...
ssize_t rb_driver_read(struct file *file, char *buf, size_t count, loff_t *ppos)          //Read function
{
	...
	if(!rb_device->current_traversal) 
	{																											//if current traversal is null for the very first time, then we need to start reading from first node of tree
		...
		kp_rbtree_sniff(rb_device, read_buff,count);
	...
	{	
		if(!rb_prev(rb_device->current_traversal)) 
		{																				//If we've reached the end of the tree(leaf node)				
			printk("No data to read\n");
			kp_rbtree_sniff(rb_device, read_buff,count);
			return -1;
		} 
...
EXPORT_SYMBOL_GPL(rb_driver_read);
