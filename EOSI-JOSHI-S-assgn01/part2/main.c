...
int main(void)
{
  int error;
	...
	// create a krpobe to get the kernel debugging
	error = pthread_create(&(krpobe_tid), NULL, &krpobe_callback, krpobe_path);

	for(i=0;i<4;i++) {
		pthread_join(tid[i], NULL);
	}

	pthread_join(krpobe_tid, NULL);
